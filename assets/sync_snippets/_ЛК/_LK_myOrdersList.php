...
if (is_numeric($_GET['confirmShipId'])) {
   ...
   /*     
        $sqlUpdateDisputeSel = "SELECT id FROM  ".$modx->getFullTableName( '_dispute_list' )."  WHERE order_id = '{$idorder}' ";
		$resultUpdateDisputeSel = mysql_query($sqlUpdateDisputeSel) or die ("Error: 534".mysql_error());
        if ($resultUpdateDisputeSel && $resultUpdateDisputeSel > 0 ) {
            if ($lastUpdId = mysql_fetch_assoc($resultUpdateDisputeSel)){
                
                $lastUpdId = $lastUpdId['id'];
                
                $sqlUpdateDispute = "UPDATE  ".$modx->getFullTableName( '_dispute_list' )." SET status = 'closed' WHERE order_id = '{$idorder}' ";
	           	$resultUpdateDispute = mysql_query($sqlUpdateDispute) or die ("Error: 3456".mysql_error());
