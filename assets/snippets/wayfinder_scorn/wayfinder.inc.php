...
class ScornWayfinder {
	...
	var $placeHolders = array(
		'rowLevel' => array('[+wf.wrapper+]','[+wf.classes+]','[+wf.classnames+]','[+wf.link+]','[+wf.title+]','[+wf.linktext+]','[+wf.id+]','[+wf.alias+]','[+wf.attributes+]','[+wf.docid+]','[+wf.introtext+]','[+wf.description+]','[+wf.subitemcount+]',
						'[+wf.linkclass+]'),
		'wrapperLevel' => array('[+wf.wrapper+]','[+wf.classes+]','[+wf.classnames+]',			'[+wf.open+]'),
		'tvs' => array(),
	);
	var $tvList = array();
	var $debugInfo = array();
		...
		foreach ($this->docs as $level => $subDocs) {
			...
			foreach ($subDocs as $parentId => $docs) {
				...
				if (!$this->_config['hideSubMenus'] || $this->isHere($parentId) || $level <= 1) {
					...
					$menuPart = $this->buildSubMenu( $docs, $level, $this->isHere($parentId) ); // scorN
	...
	function buildSubMenu($menuDocs,$level,$open) {
		global $modx;
		$subMenuOutput = '';
		$firstItem = 1;
		$counter = 1;
		...
		foreach ($menuDocs as $docId => $docInfo) {
			if( count( $this->docs[ $level+1 ][ $docInfo[ 'id' ] ] ) == 0 ){ $docInfo['empty']= 1; } // scorN
		...
		if ($level > 0 && $subMenuOutput ) { // scorN
			...
			if( $this->_css[ 'open' ] && $open ) $openClass= ' class="'. $this->_css[ 'open' ] .'"'; // scorN
			
			$phArray = array($subMenuOutput,$useClass,$classNames,$openClass);
			//Process the wrapper
    ...
    function renderRow(&$resource,$numChildren) {
        ...
        $classNames = $this->setItemClass('rowcls',$resource['id'],$resource['first'],$resource['last'],$resource['level'],$resource['isfolder'],$resource['type'],$resource['empty']);
        if ($classNames) $useClass = ' class="' . $classNames .'"';
		...
		if( $this->_css[ 'link' ] ) $linkClass= ' class="'. $this->_css[ 'link' ] .'"'; // scorN
		
		//Load row values into placholder array
        $charset = $modx->config['modx_charset'];
		if ($this->_config['entityEncode']) {
			$phArray = array($useSub,$useClass,$classNames,$resource['link'],htmlspecialchars($resource['title'], ENT_COMPAT, $charset),htmlspecialchars($resource['linktext'], ENT_COMPAT, $charset),$useId,$resource['alias'],$resource['link_attributes'],$resource['id'],htmlspecialchars($resource['introtext'], ENT_COMPAT, $charset),htmlspecialchars($resource['description'], ENT_COMPAT, $charset),$numChildren, $linkClass);
		} else {
			$phArray = array($useSub,$useClass,$classNames,$resource['link'],$resource['title'],$resource['linktext'],$useId,$resource['alias'],$resource['link_attributes'],$resource['id'],$resource['introtext'],$resource['description'],$numChildren, $linkClass);
		}
    ...
    function setItemClass($classType, $docId = 0, $first = 0, $last = 0, $level = 0, $isFolder = 0, $type = 'document', $empty = 0) {
        global $modx;
        $returnClass = '';
        $hasClass = 0;
        ...
        if ($classType === 'outercls' && !empty($this->_css['outer'])) {
            ...
            if ( !empty($this->_css['empty']) && $empty ) { // scorN
                $returnClass .= $hasClass ? ' ' . $this->_css['empty'] : $this->_css['empty'];
                $hasClass = 1;
            }
        }
	...
	function getData() {
		...
		}elseif (!$this->_config['hideSubMenus']) {
		...
		if (!empty($ids)) {
			...
			$arr_onlyForReg= explode( ",", $this->_config[ 'onlyForReg' ] );
			...
			for($i=0;$i<$numResults;$i++)  {
				...
				if( ! $this->_config[ 'only' ] || empty( $this->_config[ 'only' ] ) || $level <= $this->_config[ 'onlyStart' ] ||
					( $this->_config[ 'only' ] == 'folders' && $tempDocInfo[ 'isfolder' ] == '1' ) ||
					( $this->_config[ 'only' ] == 'items' && $tempDocInfo[ 'isfolder' ] == '0' ) )	// scorN
				{
				}else{
					$tempDocInfo= null; // scorN
				}
				
				if( ! in_array( $tempDocInfo[ 'id' ], $arr_onlyForReg ) || $modx->userLoggedIn() )	// scorN
				{
				}else{
					$tempDocInfo= null; // scorN
				}
        ...
        foreach ($this->_templates as $n => $v) {
            ...
            if (empty($v) || !$templateCheck) {
                if ($n === 'outerTpl') {
                    ...
                    $this->_templates[$n] = '<div[+wf.open+]><ul[+wf.classes+]>[+wf.wrapper+]</ul></div>';
                } elseif ($n === 'rowTpl') {
                    $this->_templates[$n] = '<li[+wf.id+][+wf.classes+]><a[+wf.linkclass+] href="[+wf.link+]" title="[+wf.title+]" [+wf.attributes+]>[+wf.linktext+]</a>[+wf.wrapper+]</li>';
