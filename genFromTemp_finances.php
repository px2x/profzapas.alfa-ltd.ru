...
$order_id = addslashes($_GET["recID"]);
...
//get order info
$sql = "SELECT * FROM ".TAB_PREFIX."_points_up WHERE id = ".$order_id." LIMIT 1";
$result = mysql_query($sql);
...
ИНН: '.$userInfo['inn'].' КПП: '.$userInfo['kpp'];
    ...
    $templateProcessor->setValue('orderNumber', $orderInfo['id']+1000);
    $templateProcessor->setValue('orderDate' ,  date("d.m.y H:i", $orderInfo['timest']));
...
//    
    ...
    $templateProcessor->setValue('positionNumber', '1');
    $templateProcessor->setValue('nameItem' , "Пополнение внутреннего счета");
    $templateProcessor->setValue('unitsMetr' , '-');
    $templateProcessor->setValue('counts' , '-');
    $templateProcessor->setValue('oneItemPrice' , '-');
    $templateProcessor->setValue('sumItemPrice' , htmlspecialchars($orderInfo['sum']));
   ...
   // $sumItem = $sumItem + $orderInfo['price_shipment'];
    ...
    $templateProcessor->setValue('sumAllPrice', htmlspecialchars($orderInfo['sum']));
    $templateProcessor->setValue('sumNDS' ,  '-');
    $templateProcessor->setValue('sumPlusNds' , htmlspecialchars($orderInfo['sum']));
}
...
header("Content-Disposition:attachment;filename='schet_".$orderInfo['sum'].'_'.date('d.m.y__H.i.s',$orderInfo['timest']).".docx'");
